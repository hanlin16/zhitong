(function ($) {
    function Scroller(elem, settings) {
        ...
        */
        that.init = function (ss) {
            ...
            extend(s, theme.defaults, lang, settings); //扩展s包含所有的设置参数
            that.settings = s;//赋值给that.settings

            // Unbind all events (if re-init)
            elm.unbind('.dw');

            var preset = ms.presets[s.preset];//时间设置
    ...
    function init(that, method, args) {
    	//如果参数method是methods中的某个个方法
        if (methods[method]) {
        	//用slice，截取args第一个到最后一组的内容，并将这段参数传给methods[method]方法，用当前对象替换原对象
    ...
    var scrollers = {},
      ...
      //方法对象   int,enable,disable,isDisabled,isVisible,option,setValue,getInst,getValue,getValues,show,hide,destroy
        methods = {
        	//int方法
    ...
    //绑定事件对象
    $(document).bind(END_EVENT, function (e) {//绑定滑动结束事件
        if (move) {
    ...
    }).bind('mouseover mouseup mousedown click', function (e) { //绑定click事件
        if (tap) {//如果被点击
    ...
    $.fn.mobiscroll = function (method) {
        extend(this, $.mobiscroll.shorts);//扩展方法，把$.mobiscroll.方法添加到自身。
        return init(this, method, arguments);//返回执行，int()函数  ,arguments指代传来的参数
    };
...
(function ($) {
    ...
    /**
    * Format a date into a string value with a specified format.格式化时间成一个特殊格式的字符串
    ...
    */
    ...
    /**
    * Extract a date from a string value with a specified format.//从特殊格式的字符串中提取时间
