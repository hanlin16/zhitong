		...
		function o(j, c, b, f, i) {
			!1 !== N("validate", [z, c, j]) && (a(".dw-ul", z).each(function(b) {
				var d = a(this),
					...
					l = b == c || void 0 === c;
				if (!e.hasClass("dw-v")) {
					for (var I = e, m = 0, q = 0; 0 <= g - m && !I.hasClass("dw-v");) m++, I = h.eq(g - m);
		...
		B = function(k) {
			...
			if (v.match(/date/i)) {
				...
				for (i = 0; 3 > i; i++) if (i == q.y) {
					...
					s++;
					g[f.monthText] = {};
					for (h = 0; 12 > h; h++) J = B.replace(/[dy]/gi, "").replace(/mm/, 9 > h ? "0" + (h + 1) : h + 1).replace(/m/, h + 1), g[f.monthText][h] = J.match(/MM/) ? J.replace(/MM/, '<span class="dw-mon">' + f.monthNames[h] + "</span>") : J.replace(/M/, '<span class="dw-mon">' + f.monthNamesShort[h] + "</span>")
				} else if (i == q.d) {
					s++;
					g[f.dayText] = {};
